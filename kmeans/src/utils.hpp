...
class AssignClusters {
    ...
    public:
        ...
        void assign_clusters(const Eigen::MatrixXd& data, const Eigen::MatrixXd& centroids, Eigen::VectorXi& assign_indices)
        {
            ...
            if (centroids.rows() == 0) {
                throw std::runtime_error("No centroids provided!");
            }
            
            for (int point = 0; point < data.rows(); point++) {
                
                int closest_cent_idx = 0;
                double min_dist = (data.row(point) - centroids.row(0)).squaredNorm();
...
class Train {
    ...
    public:
        Train(int max_epoch = 20, double threshold = 0.01) : max_epoch(max_epoch), threshold(threshold) {
            ...
            assign_indices = Eigen::VectorXi::Zero(0);
        }
        ...
        void trainer(Eigen::MatrixXd data, int num_centroids) {
            ...
            assign_indices = Eigen::VectorXi::Zero(data.rows());
            ...
            for (int i = 0; i < max_epoch; i++) {
                ...
                double wcss = compute_wcss(data);
                std::cout << "Epoch: " << i+1 << " | WCSS: " << wcss << std::endl;
        ...
        double compute_wcss(const Eigen::MatrixXd& data) 
        {
            double wcss = 0.0;

            for (Eigen::Index i = 0; i < data.rows(); ++i) {
                int cluster = assign_indices(i);
                wcss += (data.row(i) - centroids.row(cluster)).squaredNorm();
            }
        ...
        Eigen::VectorXi get_assign_indices() {
...
class Predict {
    ...
    public:
        ...
        void assign_new_points(const Eigen::MatrixXd& new_points) {
            ...
            assign_indices = Eigen::VectorXi::Zero(new_points.rows());
            AssignClusters assign;
